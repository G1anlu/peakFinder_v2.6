...
export const listFavorites = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    // Un token appena emesso può risultare "nel futuro" per pochi secondi
    // (differenza di orologio): non facciamo crashare la pagina.
    if (error) {
      if (/jwt|token/i.test(error.message)) return { favorites: [] as FavoriteResort[] };
