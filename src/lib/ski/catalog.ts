...
import { LIFT_RESORT_OVERRIDES } from "./lift-overrides";
import { canonicalRegionName, regionCandidates } from "./regions";
...
/**
 * Regione italiana canonica: prima la località curata vicina, poi i riquadri
 * geografici. Non restituisce mai il generico "Italia" se evitabile.
 */
function guessRegion(lat: number, lng: number): string {
  ...
  for (const c of curated) {
    ...
    const region = canonicalRegionName(c.region) ?? c.region.split("—")[0]!.trim();
    if (!best || km < best.km) best = { region, km };
  }
  const candidates = regionCandidates(lat, lng);
  if (best && best.km <= 30 && candidates.some((c) => c === best!.region)) return best.region;
  if (candidates.length === 1) return candidates[0]!;
  if (best && candidates.some((c) => c === best!.region)) return best.region;
  if (candidates.length > 0) return candidates[0]!;
  return best?.region ?? "Italia";
}
...
export function buildCatalog(): Resort[] {
  ...
  for (const lift of dataset.lifts) {
    // Correzioni manuali: comprensori distinti accorpati dal dataset OSM.
    const override = LIFT_RESORT_OVERRIDES.get(lift.id);
    const rawName =
      override?.name ??
      lift.resort_name ??
      lift.resortName ??
      (lift.resort ? titleize(lift.resort) : null);
    if (!rawName) continue; // impianti non associati a un comprensorio
    const slug = override?.slug ?? normalizeName(rawName).replace(/\s+/g, "-");
    let agg = groups.get(slug);
    if (!agg) {
      agg = {
        name: rawName,
        slug,
        ...(override?.region ? { regionOverride: override.region } : {}),
        ...(override?.km !== undefined ? { kmOverride: override.km } : {}),
        count: 0,
        active: 0,
        detachable: 0,
        km: 0,
  ...
  for (const agg of groups.values()) {
    ...
    const modernPct = agg.detachable > 0 && activeLifts > 0
      ...
      : 0;
    const km = agg.kmOverride ?? agg.km ?? 0;
    ...
    const base: Resort = {
      id: agg.slug,
      name: agg.name,
      region: agg.regionOverride ?? guessRegion(roundedLat, roundedLng),
      lat: roundedLat,
      lng: roundedLng,
      altitude,
      openHours: 8,
    ...
    const merged = match
      ...
      : base;
    // La regione resta sempre una delle regioni italiane canoniche.
    const resort: Resort = {
      ...merged,
      region: agg.regionOverride ?? canonicalRegionName(merged.region) ?? base.region,
    };
  ...
  for (const c of curated) {
    ...
    resorts.push({
      ...c,
      region: canonicalRegionName(c.region) ?? guessRegion(c.lat, c.lng),
    });
  }
...
/**
 * Confronto tollerante fra regione del comprensorio e filtro scelto:
 * "Italia"/"Italy"/"IT" includono tutto il catalogo, mentre una macro-regione
 * (es. "Abruzzo") include anche le sue sotto-aree ("Abruzzo — Marsica").
 */
export function matchesRegion(resortRegion: string, selected: string): boolean {
  const sel = normalizeName(selected);
  if (!sel || sel === "tutte" || sel === "italia" || sel === "italy" || sel === "it") return true;
  const reg = normalizeName(resortRegion);
  return reg === sel || reg.startsWith(`${sel} `) || reg.includes(sel);
}
...
export function searchCatalog(query: string, limit = 40): Resort[] {
  ...
  // Nessuna corrispondenza con tutti i termini: ripiego sulla ricerca libera
  // (frase intera o singolo termine) su nome, regione e impianti.
  if (scored.length === 0) {
    for (const entry of SEARCH_INDEX) {
      const haystack = `${entry.name} ${entry.region} ${entry.lifts}`;
      let score = 0;
      if (haystack.includes(q)) score += 3;
      for (const term of terms) if (term.length >= 3 && haystack.includes(term)) score += 1;
      if (score > 0) scored.push({ resort: entry.resort, score });
    }
  }
