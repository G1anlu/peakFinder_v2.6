...
export const Route = createFileRoute("/risultati")({
  validateSearch: (raw: Record<string, unknown>) => {
    ...
    if (!parsed.success) {
      // Parametri mancanti/non validi: si rimanda alla home in beforeLoad,
      // dove il redirect viene gestito correttamente anche in SSR.
      return {
        ...searchSchema.parse({
          originLat: 0,
          originLng: 0,
  ...
  beforeLoad: ({ search }) => {
    if ("invalid" in search && search.invalid) throw redirect({ to: "/" });
  },
...
function ResultsPage() {
  ...
  // Percorsi reali (LocationIQ Directions) solo sui top 12 candidati.
  ...
  // Merge: percorsi reali dove disponibili, stima altrove.
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-5xl space-y-5 px-5 py-10">
        <div className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground">
          ...
          {googlePending
            ? "Calcolo percorsi reali su OpenStreetMap sui candidati migliori…"
